...
export function createPdfRotateAdapter(): EngineAdapter<Readonly<Record<string, unknown>>> {
  return {
    ...
    async validate(_inputs, options): Promise<readonly ValidationIssue[]> {
      ...
      if (pages !== "all" && (typeof pages !== "string" || !pages.split(",").every((part) => /^\s*\d+(?:-\d+)?\s*$/.test(part)))) {
        return [{ code: "malformed-input", field: "pages", message: "Selected pages must use all, numbers, or ranges such as 1, 3-5." }];
      }
      if (!(["90", "180", "270"] as const).includes(String(degrees) as "90" | "180" | "270")) {
        return [{ code: "malformed-input", field: "degrees", message: "Rotation must be 90, 180, or 270 degrees." }];
      }
      return [];
    },
