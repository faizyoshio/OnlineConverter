          ...
          <div className="compression-settings__main-row">
            <div className="compression-settings__modes" role="radiogroup" aria-label="Compression mode">
              ...
              <label htmlFor={qualityRadioId} className={`compression-radio-label ${mode === "quality" ? "is-selected" : ""}`}>
                ...
                <span className="compression-help-icon" title="Compression quality percentage from 1% to 100%" aria-label="Compression quality percentage from 1% to 100%">?</span>
              </label>
            </div>
            ...
            <div className="compression-settings__control-area">
              {mode === "maxFileSize" ? (
                ...
                <div className="compression-slider-container">
                  <div className="compression-slider-tooltip" style={{ left: `calc(${percent}% + ${thumbOffset}px)` }}>
                    <span>{quality}%</span>
                    <span className="compression-slider-tooltip-arrow" />
                  </div>
                  <input type="range" id={qualitySliderId} min="1" max="100" step="1" value={quality} onChange={(e) => setOption("quality", Number(e.target.value))} disabled={disabled} className="compression-slider-input" style={{ background: `linear-gradient(to right, var(--ink) 0%, var(--ink) ${percent}%, #ded4cc ${percent}%, #ded4cc 100%)` }} aria-label={`Quality ${quality}%`} />
                </div>
              )}
            </div>
          </div>

          {secondaryField && (!secondaryField.isAdvanced || isAdvancedMode) && (
            <div className="compression-settings__secondary">
              <label htmlFor={`${baseId}-secondary`} className="compression-secondary-checkbox-label">
                <input type="checkbox" id={`${baseId}-secondary`} checked={options[secondaryField.key] === true} onChange={(e) => setOption(secondaryField.key, e.target.checked)} disabled={disabled} className="compression-checkbox-input" />
