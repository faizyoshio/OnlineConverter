...
const PRESETS: readonly PresetOption[] = [
  {
    id: "dasar",
    title: "Dasar",
    description: "Kompresi dasar, kualitas tinggi",
  },
  {
    id: "sedang",
    title: "Sedang",
    description: "Kompresi baik, kualitas baik",
  },
  {
    id: "kuat",
    title: "Kuat",
    description: "Kompresi tinggi, kualitas lebih rendah",
    badge: "Terkecil",
  },
  {
    id: "kustom",
    title: "Kustom",
    description: "Ukuran file terkecil, kualitas gambar kustom",
  },
];
...
export function PdfCompressSettings({
  ...
  const selectedPreset =
    typeof options.preset === "string" && ["dasar", "sedang", "kuat", "kustom"].includes(options.preset)
      ? options.preset
      : "sedang";

  const customQuality =
    typeof options.customQuality === "number" ? options.customQuality : 80;
    ...
    <div className="pdf-compress-panel" aria-labelledby={`${baseId}-title`}>
      <div className="pdf-compress-header">
        ...
        <div className="pdf-compress-heading-text">
          <h2 id={`${baseId}-title`} className="pdf-compress-title">
            Kompres PDF
          </h2>
          <p className="pdf-compress-subtitle">
            Pilih tingkat kompresi yang Anda butuhkan untuk file PDF Anda.
          </p>
        </div>
      </div>

      <div
        className="pdf-compress-options-grid"
        role="radiogroup"
        aria-label="Tingkat kompresi PDF"
      >
        {PRESETS.map((preset) => {
          ...
          return (
            <label
              key={preset.id}
              htmlFor={radioId}
              className={`pdf-compress-card ${isSelected ? "is-selected" : ""} ${disabled ? "is-disabled" : ""}`}
            >
              <div className="pdf-compress-card__left">
                <input
                  ...
                  value={preset.id}
                  checked={isSelected}
                  onChange={() => setOption("preset", preset.id)}
                  disabled={disabled}
                  className="pdf-compress-radio"
                />
              ...
              {preset.badge ? (
                <span className="pdf-compress-badge">{preset.badge}</span>
              ) : null}
            </label>
          );
        })}
      ...
      {selectedPreset === "kustom" && (
        <div className="pdf-compress-custom-area">
          <label htmlFor={`${baseId}-custom-quality`} className="pdf-compress-custom-label">
            ...
            <input
              type="range"
              id={`${baseId}-custom-quality`}
              min="1"
              max="100"
              step="1"
              value={customQuality}
              onChange={(e) => setOption("customQuality", Number(e.target.value))}
              disabled={disabled}
              className="pdf-compress-slider"
            />
          </label>
        </div>
      )}
